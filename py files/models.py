...
def KramersKronigRx(Amp, Fs):
    """
    Single-polarization Kramers-Kronig Receiver
    ...
    phiTime = np.real(ifft(phiOmega, axis=0))

    if Nmodes == 1:
        phiTime = phiTime.reshape(phiTime.size,)
        
    return phiTime


def ModifieldGerchbergSaxton(St, At, Bt, H_Dt):
    """
    Erchberg-Saxton (GS) Error Algorithm

    :param H_Dt: função de transferência para dispersão cromática da fibra.
    :param :
    """
    try:
        St = np.array(St).flatten()
        H_Dt = np.array(H_Dt).flatten()
    except:
        raise ValueError('Impossible to convert St or Ht to a numpy array')

    # Backpropagate to TX
    Xt = np.convolve(St, H_Dt)
    # Spectrum constraint
    Xt = np.convolve(H_Dt, Xt)
